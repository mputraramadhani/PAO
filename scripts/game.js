class Game extends Phaser.Scene {
	...
	create(){		
		this.dark_gameover_popup;
		this.text_gameover_popup;
		this.b_continue_rewarded;
		this.b_end_game;
		this.b_shuffle_big;
		...
		this.b_hint = draw_button(650, 40, 'hint', this);
		// if(player_data.hint_left === 0){
		// 	this.b_hint.alpha = 0.5;
		// }
		this.b_shuffle = draw_button(560, 40, 'shuffle', this);
		// if(player_data.shuffle_left === 0){
		// 	this.b_shuffle.alpha = 0.5;
		// }
		...
		b_sound.name = 'sound';
		check_audio(b_sound);
		this.circle_hint = this.add.sprite(this.b_hint.x+35, this.b_hint.y+25, 'circle');
		this.circle_shuffle = this.add.sprite(this.b_shuffle.x+35, this.b_shuffle.y+25, 'circle');
		this.txt_hint_left = this.add.text(this.circle_hint.x, this.circle_hint.y, String(player_data.hint_left), {fontFamily: 'robotomono', fontSize: 30, align: 'center',color: '#FFFFFF'}).setOrigin(0.5);
		this.txt_shuffle_left = this.add.text(this.circle_shuffle.x, this.circle_shuffle.y, String(player_data.shuffle_left), {fontFamily: 'robotomono', fontSize: 30, align: 'center',color: '#FFFFFF'}).setOrigin(0.5);
		let sign = this.add.sprite(180, 180, 'sign');
		sign.setDepth(100);
		sign.setVisible(false);
		let arrow = this.add.sprite(this.b_shuffle.x, 140, 'arrow');
		arrow.setDepth(100);
				...
				this.tweens.add({
					...
					onComplete: (tween) => {
						if(state === 'play'){
							if(obj.name === 'hint'){
								if(player_data.hint_left > 0){
									...
									Android.showRewardedVideo("game.scene.getScene('game').add_hint();");
									// this.add_hint();
								}
								...
								if(player_data.shuffle_left > 0){
									...
									Android.showRewardedVideo("game.scene.getScene('game').add_shuffle();");
									// this.add_shuffle();
								}
						...
						if(obj.name === 'sound'){
						    ...
						    Android.levelCompleteInGame(player_data.drop_mode+1,player_data.score);
							self.scene.start('game');
						} else if(obj.name === 'restart'){
//						    Android.levelLoseInGame(player_data.drop_mode+1,player_data.score);
						...
						else if(obj.name === 'continue_rewarded'){
							Android.showRewardedVideo("game.scene.getScene('game').shuffle_popup();");
							// this.shuffle_popup();
						}
						else if(obj.name === 'end_game'){
							state = 'gameover1';
							setTimeout(gameover, 0);
						}
						else if(obj.name === 'shuffle_big'){
							self.scene.start('game');
							// this.b_shuffle_big.destroy();
        ...
        // Android.levelStartInGame(player_data.drop_mode+1);
//		alert(player_data.drop_mode+1);
		//
		...
		// 	// this.b_shuffle.setTexture("btn_shuffle");
		// 	this.txt_shuffle_left.setText(player_data.shuffle_left);
		...
		const gameoverPopup =() => {
			state = 'wait1';
			bring_piece_to_back();
			this.dark_gameover_popup = this.add.rectangle(0,0,config.width,config.height,0x000000).setOrigin(0);
			this.dark_gameover_popup.alpha = 0.9;
			this.text_gameover_popup = this.add.text(360, 400, 'CONTINUE?', {fontFamily: 'PoetsenOne', fontSize: 65, align: 'center',color: '#FFFFFF'}).setOrigin(0.5);
			// self.add.text(360, 480, 'SCORE: '+player_data.score, {fontFamily: 'PoetsenOne', fontSize: 45, align: 'center',color: '#FFFFFF'}).setOrigin(0.5);
			this.b_continue_rewarded = draw_button(360, 600, 'continue_rewarded', self);
			this.b_end_game = draw_button(360, 700, 'end_game', self);
		}
		function gameover(){
		...
		function close_shuffle_popup(){
			this.dark_gameover_popup.destroy();
			this.text_gameover_popup.destroy();
			shuffle();
		}
	}
	update_left(){
		if(player_data.shuffle_left==0){
			this.txt_shuffle_left.setText("");
			this.circle_shuffle.alpha = 0;
			this.b_shuffle.setTexture("btn_play_video");
		}
		else{
			this.txt_shuffle_left.setText(player_data.shuffle_left);
			this.circle_shuffle.alpha = 1;
			this.b_shuffle.setTexture("btn_shuffle");
		}
		if(player_data.hint_left==0){
			this.txt_hint_left.setText("");
			this.circle_hint.alpha = 0;
			this.b_hint.setTexture("btn_play_video");
		}
		else{
			this.txt_hint_left.setText(player_data.hint_left);
			this.circle_hint.alpha = 1;
			this.b_hint.setTexture("btn_hint");
		}
	...
	popup_bonus(type){
		...
		dark.alpha = 0.8;
		this.add.text(360, 320, 'BONUS', {fontFamily: 'PoetsenOne', fontSize: 65, align: 'center',color: '#FFFFFF'}).setOrigin(0.5);
		this.add.text(360, 600, '+1', {fontFamily: 'PoetsenOne', fontSize: 75, align: 'center',color: '#FFFFFF'}).setOrigin(0.5);
		if(type=="life"){
			let bonus_icon = this.add.sprite(360,480,'shuffle_icon');
			let b_next = draw_button(360, 700, 'shuffle_big', this);
		}else{
			let bonus_icon = this.add.sprite(360,480,type+'_icon');
			let b_next = draw_button(360, 700, 'next', this);
		}
	...
	add_shuffle(){
		player_data.shuffle_left++;
		this.update_left();
		this.popup_bonus("shuffle");
		Android.gameProgressEvent("show_rewarded_shuffle")
	}
	add_hint(){
		player_data.hint_left++;
		this.update_left();
		this.popup_bonus("hint");
		Android.gameProgressEvent("show_rewarded_hint")
	}
	shuffle_popup(){
		if(this.b_continue_rewarded!=null){
			this.b_continue_rewarded.destroy();
		}
		if(this.b_end_game!=null){
			this.b_end_game.destroy();
		}
		if(this.text_gameover_popup!=null){
			this.text_gameover_popup.destroy();
		}
		if(this.dark_gameover_popup!=null){
			this.dark_gameover_popup.destroy()
		}
		if(this.text_gameover_popup!=null){
			this.text_gameover_popup.destroy()
		}
		this.popup_bonus("life")
		Android.gameProgressEvent("show_rewarded_life")
		// this.b_shuffle_big = draw_button(360, 550, 'shuffle_big', this);
		// shuffle_pieces();
	}
